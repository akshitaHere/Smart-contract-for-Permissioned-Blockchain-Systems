...
class PatientRecordContract extends Contract {
   ...
   async queryByGender(ctx, gender) {
	...
	let query = {
        ...
        let listOfRecords = await this.queryWithQueryString(ctx, JSON.stringify(query));
   ...
   async queryByBlood_Type(ctx, blood_type) {
    ...
    //      Pass the Query string built to queryWithQueryString
	 let query = {
            ...
            "use_index": ["_design/blood_Type_IndexDoc", "blood_Type_Index"]
        };
        let listOfRecords = await this.queryWithQueryString(ctx, JSON.stringify(query));
        return listOfRecords;
    ...
    */
    //Grade Function
   async queryByBlood_Type_Dual(ctx, bloodType1, bloodType2) {
	...
	let query = {
            ...
            "use_index":[
               "_design/blood_Type_IndexDoc",
               "blood_Type_Index"
            ]
         };

        let listOfBloodType = await this.queryWithQueryString(ctx, JSON.stringify(query));
        return listOfBloodType;
